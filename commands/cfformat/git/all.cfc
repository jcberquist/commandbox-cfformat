/**
 * Formats script and tag components for modified files in the Git repository
 *
 * {code:bash}
 * cfformat git all
 * cfformat git all path/to/git/directory
 * {code}
 */
component accessors="true" aliases="fmt-git-all" {
    ...
    /**
     * @directory git directory path to format modified files
    ...
    function run(
        ...
        var path = getAllGitFiles(gitApi);

        if (!path.len()) {
            print.yellowLine('No non-ignored git files to format.');
            return;
        }
    ...
    string function getAllGitFiles(required gitApi) {
