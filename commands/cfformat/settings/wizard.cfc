component {
    ...
    function run() {
        ...
        var userSettings = reference.reduce( ( settings, name, options ) => {
            ...
            print.blackOnYellowLine( name ).line(options.description).line().toConsole();
            ...
            switch (options.type) {
                case 'integer':
                    settings[ name ] = toNumeric(ask( message='Enter integer value: ', defaultResponse=defaultSettings[name] ));
                    break;
                case 'boolean':
                ...
                case 'struct-key-value':
                    ...
                    .setOptions(examples[name].keyArray().map( ( choice ) => ( { display: serializeJSON(choice), "value": choice, "selected": choice == defaultSettings[name]  } ) ))
                    .setRequired( true )
                    .setMultiple( false )
                    .ask();
                    break;                   
                case 'string':
                    settings[ name ] = multiselect()
                        ...
                        .setOptions(
                            options.values.map( ( choice ) => ( { display: serializeJSON(choice), "value": choice, "selected": choice == defaultSettings[name]  } ) )
                        )
                        .setRequired( true )
                        .setMultiple( false )
                        .ask();
